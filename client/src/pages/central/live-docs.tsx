...
export function LiveDocsSection({ orders }: { orders?: NormalizedOrder[] }) {
  ...
  const searchOptions = useMemo(() => {
    ...
    orders.forEach((o) => {
      ...
      if (o.motoboyName) set.add(o.motoboyName);
    });
    ...
    return orders
      ...
      .filter((o) => {
        ...
        return (
          ...
          (o.motoboyName || "").toLowerCase().includes(term)
        );
      })
      .sort((a, b) => {
    ...
    <Card className="p-6 space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Filtre por período, pesquise por cliente/motoboy ou ID.</p>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3 w-full sm:w-auto">
          ...
          <Input
            placeholder="Buscar cliente, motoboy ou ID"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="sm:w-[220px]"
            list="live-docs-central-options"
          />
        </div>
      </div>

      <datalist id="live-docs-central-options">
