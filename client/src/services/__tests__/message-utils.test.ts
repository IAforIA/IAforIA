...
describe('message-utils (client)', () => {
  it('normalizes legacy fromId/toId to senderId/receiverId', () => {
    const legacy = { fromId: 'user-123', toId: null, fromName: 'João', fromRole: 'client' } as any;
    const normalized = normalizeIncomingChatPayload(legacy);
