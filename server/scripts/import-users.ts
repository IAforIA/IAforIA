...
async function importarUsuarios() {
  ...
  const credentialRecorder = new CredentialRecorder('import-users');
  ...
  for (const usuario of USUARIOS_EMPRESA) {
    try {
      ...
      // Hash da senha
      const tempPassword = generateSecurePassword(usuario.id);
  ...
  const credFile = credentialRecorder.finalize();
  console.log('📁 Senhas temporárias foram salvas em:', credFile);
  console.log('⚠️  Compartilhe o arquivo com segurança e exclua-o após distribuir as credenciais.');
  ...
  console.log('\n📝 Credenciais de acesso: consulte o arquivo CSV gerado para cada senha única.');
