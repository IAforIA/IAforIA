...
async function importarTudo() {
    ...
    for (const moto of MOTOBOYS) {
        try {
            const senhaHash = await bcrypt.hash('Guriri2024', 10);
            await db.insert(users).values({
                id: moto.id,
                name: moto.nome,
                role: 'motoboy',
    ...
    for (const cliente of CLIENTES) {
        try {
            const senhaHash = await bcrypt.hash('Cliente2024', 10);
    ...
    console.log(`   Senha padrão: Guriri2024`);
    console.log('   Emails:');
    MOTOBOYS.forEach(m => {
    ...
    console.log(`   Senha padrão: Cliente2024`);
