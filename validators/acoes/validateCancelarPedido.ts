...
export function validateCancelarPedido(dados: any): ValidationResult {
  const errors: string[] = [];
  ...
  if (typeof dados.reembolso !== "boolean") {
    errors.push("reembolso é obrigatório e deve ser boolean");
  }

  const allowedFields = ["pedidoID", "motivo", "reembolso"];
